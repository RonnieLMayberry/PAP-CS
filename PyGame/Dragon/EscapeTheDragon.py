...
#repeating loop
while True:
    milliseconds = framerate.tick(FPS)
    playtime += milliseconds / 1000.0
    framerate.tick(120)
    ticks = pygame.time.get_ticks()
    
    text = "FPS: {0:.2f}   Playtime: {1:.2f}".format(framerate.get_fps(), playtime)
    pygame.display.set_caption("Escape The Dragon Game  " + str(text))
    ...
    #is the player holding down the key?
    if keys[K_SPACE]:
        player.Y += jump_vel
        jump_vel += 0.0
        if player.Y > player_start_y:
    ...
    #draw sprites
    group.draw(screen)
    print_text(font, 350, 540, "Press SPACE to jump!")
    print_text(font, 350, 560, "Hold SPACE to jump higher!")
    print_text(font, 350, 580, "Score: " + str(score))
